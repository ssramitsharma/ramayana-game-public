...
flowchart LR
  subgraph Godot[Godot 4 Game]
    ENV["Headless RL runner (stdin/stdout JSON)"]
    LIVE["Live TCP bridge (127.0.0.1:5555)"]
    SIM["Game simulation (Ram scripted, Rakshasa controlled)"]
    ENV --> SIM
    LIVE --> SIM
  end

  subgraph Python[Python]
    PPO["PPO policy (Stable-Baselines3)"]
